...
namespace SoulboundMod.Spirit.Components
{
    public class SpiritController : MonoBehaviour
    {
        ...
        public void EnterAttackMode()
        {
            if (Util.HasEffectiveAuthority(owner))
            {
                ...
                EffectManager.SimpleEffect(SoulboundAssets.bloodExplosionEffect, base.transform.position, Quaternion.identity, false);
            }
        ...
        {
            this.bodyMachine.SetInterruptState(new SpiritLunge{ victimBody = victimBody }, EntityStates.InterruptPriority.PrioritySkill);
        }

        public void Redirect(Vector3 position, float minDistance, bool empower = true)
        {
            this.bodyMachine.SetInterruptState(new SpiritRedirect{ travelPosition = position, minDistanceFromPoint = minDistance, isEmpower = empower }, EntityStates.InterruptPriority.PrioritySkill);
        }
